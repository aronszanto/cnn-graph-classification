...
def wl_kernel(g1, g2=None, h=1):
    ...
    orig_graph_map = {
        it: {i: defaultdict(lambda: 0) for i in range(N)} for it in range(-1, h)}
    ...
    for G in graphs:
        ...
        for node in G.nodes():
            ...
            orig_graph_map[-1][ind][label] = orig_graph_map[-1][
                ind].get(label, 0) + 1
            ...
            for node in G.nodes():
                ...
                if len(list(neighbors)) > 0:
                    neighbors_label = tuple(
                        [labels[ind][node2index[neigh]] for neigh in neighbors])
                    node_label = str(node_label) + "-" + \
                        str(sorted(neighbors_label))
                if not (node_label in label_lookup):
                    label_lookup[node_label] = len(label_lookup)

                compressed_labels[ind][node2index[
                    node]] = label_lookup[node_label]
                orig_graph_map[it][ind][node_label] = orig_graph_map[
                    it][ind].get(node_label, 0) + 1
            ...
            for i in range(len(g1)):
                for j in range(len(g2)):
                    common_keys = set(orig_graph_map[it][i].keys()) & set(
                        orig_graph_map[it][len(g1) + j].keys())
                    K[i][j] += sum([orig_graph_map[it][i].get(k, 0) * orig_graph_map[
                                   it][len(g1) + j].get(k, 0) for k in common_keys])
    else:
        K = np.zeros((N, N))
        for it in range(-1, h):
            for i in range(N):
                for j in range(N):
                    ...
                    K[i][j] += sum([orig_graph_map[it][i].get(k, 0) *
